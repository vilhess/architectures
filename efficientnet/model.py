...
base_model = [
    # expands_ration, channels, repeats, stride, kernel_size
    [1, 16, 1, 1, 3],
    [6, 24, 2, 2, 3],
    [6, 40, 2, 2, 5],
    [6, 80, 3, 2, 3],
...
class InvertedResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, expand_ratio, reduction=4, survival_prob=0.8):
        ...
        self.conv = nn.Sequential(
            CNNBlock(hidden_dim, hidden_dim, kernel_size=kernel_size,
...
class EfficientNet(nn.Module):
    ...
    def create_features(self, width_factor, depth_factor, last_channels):
        ...
        features = [CNNBlock(CHANNELS, channels, 3, stride=2, padding=1)]
...
def test():
    ...
    x = torch.randn((num_examples, CHANNELS, res, res)).to(device)
    model = EfficientNet(
        version=version,
