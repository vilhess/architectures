    ...
    def forward(self, src, trg):
        ...
        src_positions = (
            torch.arange(0, src_seq_length).unsqueeze(
                1).expand(src_seq_length, N).to(self.device)
        )
        trg_positions = (
            torch.arange(0, trg_seq_length).unsqueeze(
                1).expand(trg_seq_length, N).to(self.device)
        )
        ...
        out = self.transformer(
            embed_src, embed_trg, src_key_padding_mask=src_padding_mask, tgt_mask=trg_mask
        )
...
if load_model:
    load_checkpoint(torch.load("my_checkpoints.pth.tar"), model, optimizer)
